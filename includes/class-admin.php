...
class YTCT_Admin {
	...
	public function add_menu_page() {
		add_options_page(
			__('YT Consent Translations', 'yt-consent-translations-main'),
			__('YT Consent Translations', 'yt-consent-translations-main'),
			'manage_options',
	...
	public function enqueue_assets($hook) {
		...
		wp_localize_script('ytct-admin-script', 'ytctAdmin', [
			...
			'strings' => [
				'saving' => __('Saving...', 'yt-consent-translations-main'),
				'saved' => __('Settings saved successfully!', 'yt-consent-translations-main'),
				'error' => __('An error occurred. Please try again.', 'yt-consent-translations-main'),
				'confirmReset' => __('Are you sure you want to reset all settings for this locale scope to defaults?', 'yt-consent-translations-main'),
				'resetting' => __('Resetting...', 'yt-consent-translations-main'),
				'resetSuccess' => __('Settings reset successfully!', 'yt-consent-translations-main'),
				'importing' => __('Importing...', 'yt-consent-translations-main'),
				'importSuccess' => __('Settings imported successfully!', 'yt-consent-translations-main'),
				'invalidFile' => __('Please select a valid JSON file.', 'yt-consent-translations-main'),
				'languageLoaded' => __('Language preset loaded!', 'yt-consent-translations-main'),
				'scopeLoaded' => __('Locale scope loaded.', 'yt-consent-translations-main'),
				'qualityCheckRunning' => __('Running quality checks...', 'yt-consent-translations-main'),
				'qualityCheckOk' => __('No blocking quality issues found.', 'yt-consent-translations-main'),
				'healthCheckRunning' => __('Running compatibility health check...', 'yt-consent-translations-main'),
				'healthCheckOk' => __('Compatibility check completed.', 'yt-consent-translations-main'),
				'restored' => __('Snapshot restored successfully.', 'yt-consent-translations-main'),
				'unsavedChanges' => __('You have unsaved changes. Leave without saving?', 'yt-consent-translations-main'),
				'selectSnapshot' => __('Select a snapshot', 'yt-consent-translations-main'),
				'selectSnapshotFirst' => __('Select a snapshot first.', 'yt-consent-translations-main'),
				'qualityCheckFailed' => __('Quality check reported issues/warnings.', 'yt-consent-translations-main'),
				'checkUpdateRunning' => __('Checking WordPress.org update metadata...', 'yt-consent-translations-main'),
	...
	private function verify_ajax_request() {
		...
		if (empty($nonce) || !wp_verify_nonce($nonce, 'ytct_admin_nonce')) {
			wp_send_json_error(['message' => __('Security check failed.', 'yt-consent-translations-main')]);
		}

		if (!current_user_can('manage_options')) {
			wp_send_json_error(['message' => __('Permission denied.', 'yt-consent-translations-main')]);
		}
	}
	...
	private function build_quality_report($strings, $preset_translations) {
		...
		if (!empty($invalid_placeholder_fields)) {
			$issues[] = sprintf(
				...
				__('The following fields must include %1$s or %2$s: %3$s', 'yt-consent-translations-main'),
				'%s',
				'%1$s',
		...
		foreach ($strings as $key => $value) {
			...
			if ($value !== '' && substr_count($value, '<a ') !== substr_count($value, '</a>')) {
				$warnings[] = sprintf(
					...
					__('%s may contain malformed anchor HTML.', 'yt-consent-translations-main'),
			...
			if ($value !== '' && $reference !== '' && strlen($reference) > 40) {
				...
				if ($ratio > 1.8) {
					$warnings[] = sprintf(
						/* translators: %s field label */
						__('%s is much longer than the preset and may overflow on small screens.', 'yt-consent-translations-main'),
						YTCT_Strings::get_key_label($key)
					);
				}
			}
		...
		foreach ($pairs as $pair) {
			...
			if ($left !== '' && $right !== '' && $left_cmp === $right_cmp) {
				$warnings[] = sprintf(
					...
					__('Fields %1$s and %2$s are identical. Consider using distinct labels for clarity.', 'yt-consent-translations-main'),
	...
	public function ajax_save_settings() {
		...
		wp_send_json_success([
			'message' => __('Settings saved successfully!', 'yt-consent-translations-main'),
			'scope' => $scope_payload
		]);
	}
	...
	public function ajax_reset_settings() {
		...
		wp_send_json_success([
			'message' => __('Settings reset successfully!', 'yt-consent-translations-main'),
	...
	public function ajax_import_settings() {
		...
		if (!isset($_FILES['import_file'])) {
			wp_send_json_error(['message' => __('No file uploaded.', 'yt-consent-translations-main')]);
		}
		...
		if (!isset($ytct_file['error']) || $ytct_file['error'] !== UPLOAD_ERR_OK) {
			wp_send_json_error(['message' => __('File upload failed.', 'yt-consent-translations-main')]);
		}

		$max_size = 150 * 1024;
		if (!isset($ytct_file['size']) || $ytct_file['size'] > $max_size) {
			wp_send_json_error(['message' => __('File too large. Maximum size is 150KB.', 'yt-consent-translations-main')]);
		}

		if (!isset($ytct_file['name'])) {
			wp_send_json_error(['message' => __('Invalid file.', 'yt-consent-translations-main')]);
		}
		...
		if (!$file_info['ext'] || !isset($allowed_extensions[strtolower($file_info['ext'])])) {
			wp_send_json_error(['message' => __('Invalid file type. Only JSON files are allowed.', 'yt-consent-translations-main')]);
		}

		if (!isset($ytct_file['tmp_name']) || !is_uploaded_file($ytct_file['tmp_name'])) {
			wp_send_json_error(['message' => __('Invalid file upload.', 'yt-consent-translations-main')]);
		}
		...
		if (json_last_error() !== JSON_ERROR_NONE || !is_array($data)) {
			wp_send_json_error(['message' => __('Invalid JSON file.', 'yt-consent-translations-main')]);
		}
		...
		wp_send_json_success([
			'message' => __('Settings imported successfully!', 'yt-consent-translations-main'),
			'scope' => $this->build_scope_payload($scope_locale),
			'options' => $stored
		]);
	}
	...
	public function ajax_restore_snapshot() {
		...
		if ($snapshot_id === '') {
			wp_send_json_error(['message' => __('Snapshot ID is required.', 'yt-consent-translations-main')]);
		}

		$restored = YTCT_Options::restore_snapshot($snapshot_id, $scope_locale);
		if (!is_array($restored)) {
			wp_send_json_error(['message' => __('Snapshot could not be restored.', 'yt-consent-translations-main')]);
		}
		...
		wp_send_json_success([
			'message' => __('Snapshot restored successfully.', 'yt-consent-translations-main'),
			'scope' => $this->build_scope_payload($scope_locale),
			'options' => $restored
		]);
	}
	...
	public function ajax_check_update_now() {
		...
		$message = __('No new version found. Plugin is up to date.', 'yt-consent-translations-main');
		if (!empty($updater['status']) && $updater['status'] === 'error') {
			$message = __('Update check completed with an error. Check updater status.', 'yt-consent-translations-main');
		} elseif (!empty($updater['updateAvailable'])) {
			$message = __('A new version is available. Update it from the Plugins screen.', 'yt-consent-translations-main');
		}
